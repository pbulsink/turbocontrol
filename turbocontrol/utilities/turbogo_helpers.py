...
def get_dielectrics(inputfile):
    ...
    #make path first?
    readfile = os.path.join(os.path.dirname(os.path.realpath(os.getcwd())), inputfile)
    ...
    try:
        ...
        try:
            with open('dielectricsolvents.csv') as f:
                data = f.readlines()
        except Exception as e:
            print "Error reading file {}.".format(readfile)
            raise FileAccessError("Error reading file {}.".format(readfile), e)
